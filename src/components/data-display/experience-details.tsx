...
const ExperienceDetails = ({
  ...
  width,
  height,
}: ExperienceDetailsProps & { width?: number; height?: number }) => { // Adicionando ao tipo
  return (
      ...
      <div className="max-md:order-2 md:w-1/4">
        {startDate ? (
          <Typography className="text-gray-700 md:text-right">
            {new Intl.DateTimeFormat('pt-BR', dateFormatOptions).format(startDate)}
            {' - '}
            {currentlyWorkHere
              ? 'Presente'
              : endDate
              ? new Intl.DateTimeFormat('pt-BR', dateFormatOptions).format(endDate)
              : 'NA'}
          </Typography>
        ) : null} 
      </div>
